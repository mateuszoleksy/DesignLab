...
while (1):
	...
	val = soup.find(config['temp_page']['element'],id=config['temp_page']['id']).text
	now = datetime.now()
	...
	mydbcursor.execute('DELETE FROM dev_temp WHERE date <= now() - interval '+config['interval']['dev_temp_time']+' minute')
	mydb.commit()
	val = 0
	...
	try:
		...
		try:
			...
			mydbcursor.execute(sql, tab)
			#usun stare rekord, domyslnie jest powyzej 1 minuty usuwane
			mydbcursor.execute('DELETE FROM lab_temp WHERE date <= now() - interval '+config['interval']['dev_temp_time']+' minute')
			mydb.commit()
